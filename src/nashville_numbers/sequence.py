    ...
    for section_index, section in enumerate(plan["sections"]):
        for bar_index, bar in enumerate(section["bars"]):
            ...
            for slot_index, slot in enumerate(bar["slots"]):
                ...
                start_ms = round((bar_start_beats + slot["beat_start"]) * beat_ms)
                duration_ms = max(
                    180,
                    round(slot["beat_duration"] * beat_ms * groove["gate"]),
                )

                velocity = 82 if groove["id"] == "lantern" else 96
                events.append({
                    "kind": "chord",
                    "delay_ms": start_ms,
                ...
                if plan.get("bass_enabled", True):
                    events.extend(
                        _build_bass_events(slot, chord_data, key, groove, start_ms, beat_ms)
                    )
                ...
                highlights.append({
                    "key": slot_key,
                    "delay_ms": start_ms,
                    "duration_ms": max(220, duration_ms),
                })
